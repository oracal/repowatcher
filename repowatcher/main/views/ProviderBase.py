...
class ProviderBase(object):
    ...
    def save_repositories(self, repositories_dict, owned):
                ...
                if item !='':
                    ...
                    if update or (datetime.now() - repository.last_modified) > timedelta(days = 1):
                        ...
                        if not repository.private:
                            repository.save()
                    try:
                        user_repository_link = UserRepositoryLink.objects.get(user = profile,repository = repository,owned = owned)
                    except ObjectDoesNotExist:
                        user_repository_link = UserRepositoryLink(user = profile,repository = repository,owned = owned)
    ...
    def save_watched_repositories(self, repositories_dict):
        self.save_repositories(owned = False, repositories_dict = repositories_dict)

    def save_owned_repositories(self, repositories_dict):
        self.save_repositories(owned = True, repositories_dict = repositories_dict)

    def retrieve_repositories_list(self, username, owned):
        repositories = []
        api_only = True
        if self.user.is_authenticated():
            try:
                if username == self.user.social_auth.get(provider=self.host).extra_data['username']:
                    profile = self.user.get_profile()
                    links = profile.userrepositorylink_set.filter(owned=owned).filter(repository__host=self.host).select_related('repository').select_related('repository','repository_category')
                    if len(links) != 0:
                        for link in links:
    ...
    def retrieve_repositories_dict(self, username, owned):
    ...
    def retrieve_watched_category_repositories(self, username, category):
        return self.retrieve_category_repositories(username = username, owned = False, category = category)

    def retrieve_owned_category_repositories(self, username, category):
        return self.retrieve_category_repositories(username = username, owned = True, category = category)
    
    def retrieve_category_repositories(self, username, category, owned):
        watched_filtered = []
        ...
        if self.user.is_authenticated():
            try:
                if username == self.user.social_auth.get(provider=self.host).extra_data['username']:
                    profile = self.user.get_profile()
                    links = profile.userrepositorylink_set.filter(owned = owned).filter(repository__host=self.host).filter(repository_category__name__iexact=category).select_related('repository','repository_category')
                    if len(links)!=0:
                        for link in links:
    ...
    def update_repositories(self, username, owned):
        ...
        watched = list(self.get_repositories(username, owned))
        profile = self.user.get_profile()
        # Removes repisotory links that are no longer watched from the api
        saved = Repository.objects.filter(userrepositorylink__user=profile).filter(userrepositorylink__owned=owned).filter(host=self.host)
        ...
        for slug in removed_slugs:
            ...
            UserRepositoryLink.objects.get(user = profile, repository = repository, owned = owned).delete()

        # finds the highest order value for each category in user repository links
        category_query = RepositoryCategory.objects.filter(userrepositorylink__user = profile).filter(userrepositorylink__owned=owned).distinct()
        order_category = {}
        for category in category_query:
            user_repositories = UserRepositoryLink.objects.filter(user=profile).filter(owned=owned).filter(repository_category=category)
        ...
        for repo in watched:
            ...
            try:
                ...
                user_repository_link = UserRepositoryLink(user = profile,repository = repository, order = order_category[repository.language.lower()], owned = owned)
...
#                RepositoryUserRepositoryLink.objects.get_or_create(user = repository_user, repository = repository, owned = owned)
    ...
    def update_watched_repositories(self, username):
        return self.update_repositories(username, owned=False)

    def update_owned_repositories(self, username):
        return self.update_repositories(username, owned=True)
    ...
    def update_category_repositories(self, username, category, owned):
        ...
        # Removes repisotory links that are no longer watched from the api
        saved = Repository.objects.filter(userrepositorylink__user=profile).filter(userrepositorylink__repository_category = category_object).filter(host=self.host)
        saved_slugs = (e.slug for e in saved)
        saved_slugs_set = set(saved_slugs)
        watched_slugs =(e['owner'] + '/' + e['name'] for e in watched)
        watched_slugs_set = set(watched_slugs)
        ...
        for slug in removed_slugs:
            repository = Repository.objects.get(host_slug = self.host+'/'+slug)
            UserRepositoryLink.objects.get(user = profile, repository = repository).delete()

        # finds the highest order value for the category in user repository links
        order = UserRepositoryLink.objects.filter(user=profile).filter(repository_category=category_object).aggregate(max_order=Max('order'))['max_order']
        ...
        for repo in watched:
            ...
            try:
                ...
                if repository.language == category_lower:
                    ...
                    user_repository_link = UserRepositoryLink(user = profile,repository = repository, order = order, owned = owned)
                    user_repository_link.repository_category,_ = RepositoryCategory.objects.get_or_create(name = repository.language)
                    user_repository_link.save()
#            if not repository.private:
#                RepositoryUserRepositoryLink.objects.get_or_create(user = repository_user, repository = repository, owned = owned)
    ...
    def update_watched_category_repositories(self, username, category):
        return self.update_category_repositories(username, category, owned=False)

    def update_owned_category_repositories(self, category, username):
        return self.update_category_repositories(username, category, owned=True)
